...
public class Belt {
	...
	public synchronized void identifyTagged() 
		...
		while (segment[Params.SENSOR_POSITION] == null||segment[Params.SENSOR_POSITION].hasInspected) {
			wait();
		}
	...
	/**
	 * robot takes off a tagged bicycle from the belt 
	 * or put inspected bicycle back into the belt.
	 * 
	 ...
	 *             if the thread executing is interrupted.
	 */
	public synchronized void takeOff_goBack() 
			throws InterruptedException,SensorException {
		// if there is a bicycle waiting for being taken off from belt,
		// and there is no bicycle in inspector.
		if(robotTakeOff && goBackBicycle==null){
			takeOff();
		}
		//if a bicycle has been inspected and need to be put back.
		else if(goBackBicycle != null && segment[Params.SENSOR_POSITION] == null){
			goBack();
		}
		//other situation, just wait
		else{
			wait();
		}
		// notify any waiting threads that the belt has changed
	...
	/**
	 * robot puts back the inspected bicycle.
	 * 
	 * @throws InterruptedException
	 *             if the thread executing is interrupted.
	 */
	public synchronized void goBack() 
			throws InterruptedException {
		// insert the element at the specified location
		segment[Params.SENSOR_POSITION] = goBackBicycle;

		// make a note of the event in output trace
		System.out.println(indentation+ "Robot: after inspected " + goBackBicycle + "goes back into belt");
	...
	public synchronized void takeOff() 
		...
		//take off a tagged bicycle from the belt
			...
			System.out.println(indentation+"Robot: robot takes off bicycle " + segment[Params.SENSOR_POSITION].id);
